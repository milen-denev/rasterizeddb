...
fn bench_get_throughput(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000, 1000000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("100_percent_hit", size), size, |b, _| {
            b.iter(|| {
                for key in &keys {
        ...
        group.bench_with_input(BenchmarkId::new("50_percent_hit", size), size, |b, _| {
            b.iter(|| {
                        ...
                        .map(|thread_id| {
                            ...
                            thread::spawn(move || {
                                ...
                                for i in 0..ops_per_thread {
                                    ...
                                    if op < 7 {
                                        let _ =
                                            cache_clone.insert(black_box(key), black_box(key * 2));
                                    } else if op < 9 {
...
fn bench_memory_pressure_throughput(c: &mut Criterion) {
    ...
    for (cache_size, insert_count) in [(1000, 10000), (5000, 50000), (10000, 100000)].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new(
                "eviction_pressure",
                format!("{}_{}", cache_size, insert_count),
            ),
...
fn bench_bulk_throughput(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000].iter() {
        ...
        group.bench_with_input(BenchmarkId::new("get_all_by_keys", size), size, |b, _| {
            b.iter(|| {
                    ...
                    let handles: Vec<_> = (0..threads)
                        .map(|thread_id| {
                            ...
                            thread::spawn(move || {
                                for i in 0..ops_per_thread {
                                    let key_idx =
                                        (thread_id * ops_per_thread + i) % hot_keys_clone.len();
