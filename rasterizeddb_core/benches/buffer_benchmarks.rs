...
use rasterizeddb_core::simds::endianess::{read_u64, read_u128};
...
fn criterion_benchmark_buffers(c: &mut Criterion) {
    ...
    c.bench_function("get_read_u64_raw", |b| {
        b.iter(|| {
            get_read_u64_raw(black_box(_8_ptr));
        })
    });

    c.bench_function("get_read_u64_crate", |b| {
        b.iter(|| get_read_u64_crate(black_box(&data_8)))
    });

    c.bench_function("get_read_u128_raw", |b| {
        b.iter(|| {
            get_read_u128_raw(black_box(_16_ptr));
        })
    });

    c.bench_function("get_read_u128_crate", |b| {
        b.iter(|| get_read_u128_crate(black_box(&data_16)))
    });
}
