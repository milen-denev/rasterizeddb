...
struct ColumnWindowRuleBuilder {
    ...
    seq: u64,
    pending_rule: Option<SemanticRule>,
...
impl ColumnWindowRuleBuilder {
    ...
    fn flush_window(&mut self, table_name: &str, start_row_id: u64, end_row_id: u64) {
        ...
        // RLE: Try to merge with pending rule if bounds are identical.
        if let Some(ref mut pending) = self.pending_rule {
            if pending.lower == lower && pending.upper == upper {
                // Lossless merge: extend the pending rule to cover this window too.
                pending.end_row_id = end_row_id;
                self.reset_window();
                return;
            } else {
                // Cannot merge: flush pending and start new.
                self.rules.push(pending.clone());
        ...
        self.pending_rule = Some(SemanticRule {
            rule_id,
    ...
    fn finish(&mut self) {
        if let Some(pending) = self.pending_rule.take() {
            self.rules.push(pending);
        }
    }
...
struct StringWindowRuleBuilder {
    ...
    pending_rules: [Option<SemanticRule>; 16],
    rules: Vec<SemanticRule>,
}
...
impl StringWindowRuleBuilder {
    ...
    fn flush_window_rules(&mut self, start_row_id: u64, end_row_id: u64) {
        ...
        let mut push_rule = |op: SemanticRuleOp, lower: [u8; 16], upper: [u8; 16]| {
            ...
            // RLE: Try to merge with pending rule for this Op.
            if let Some(ref mut pending) = self.pending_rules[op_idx] {
                if pending.lower == lower && pending.upper == upper {
                    // Lossless merge
                    pending.end_row_id = end_row_id;
                    return;
                    ...
                    // Flush pending
                    self.rules.push(pending.clone());
                    self.pending_rules[op_idx] = None;
                }
            }

            // Create new
            let rule_id = make_rule_id(&self.table_name, self.column_schema_id, self.seq);
            self.seq = self.seq.wrapping_add(1);

            self.pending_rules[op_idx] = Some(SemanticRule {
                rule_id, // Note: rule_id is unique per emitted rule, not per window batch anymore
    ...
    fn finish(&mut self) {
        for p in self.pending_rules.iter_mut() {
            if let Some(pending) = p.take() {
                self.rules.push(pending);
            }
        }
    }
