...
impl<'a> IOOperationsAsync<'a> for LocalStorageProviderAsync {
    async fn write_data(&'a mut self, position: u64, buffer: &[u8]) {
    ...
    async fn append_data(&'a mut self, buffer: &'a [u8]) {
    ...
    async fn append_data_own(&'a mut self, buffer: Box<Vec<u8>>) {
        let file = &mut self.append_file;
        file.write_all(&buffer).await.unwrap();
        file.flush().await.unwrap();
        file.sync_all().await.unwrap();
    ...
    async fn write_data_own(&'a mut self, position: u64, buffer: Box<Vec<u8>>) {
        let file = &mut self.append_file;
        file.seek(SeekFrom::Start(position)).await.unwrap();
    ...
    async fn read_data(&'a mut self, position: u64, length: u32) -> Vec<u8> {
