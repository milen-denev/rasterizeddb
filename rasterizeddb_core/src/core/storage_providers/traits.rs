...
pub trait StorageIO: Clone + Sync + Send + 'static {
    fn write_data_unsync(&self, position: u64, buffer: &[u8]) -> impl Future<Output = ()> + Send + Sync;

    fn verify_data(&self, position: u64, buffer: &[u8]) -> impl Future<Output = bool> + Send + Sync;

    fn verify_data_and_sync(&self, position: u64, buffer: &[u8]) -> impl Future<Output = bool> + Send + Sync;

    fn write_data(&self, position: u64, buffer: &[u8]) -> impl Future<Output = ()> + Send + Sync;

    fn write_data_seek(&self, seek: SeekFrom, buffer: &[u8]) -> impl Future<Output = ()> + Send + Sync;
    ...
    fn append_data(&self, buffer: &[u8], immediate: bool) -> impl Future<Output = ()> + Send + Sync;
