...
pub fn add_column_from_query_purpose(qp: &QueryPurpose) -> Option<AddColumn> {
	if let QueryPurpose::AddColumn(sql) = qp {
		...
		let column = AddColumnData { name: col_name.to_string(), data_type };
		Some(AddColumn { table_name: table_name.to_string(), column })
	} else {
		None
	}
}
...
mod tests {
	...
	fn test_add_column_all_types() {
		...
		for (col, typ, expected) in types.iter() {
			...
			assert_eq!(result.column.data_type, *expected, "Type mismatch for column {}", col);
		}
	}
