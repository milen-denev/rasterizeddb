...
use crate::{
    ...
    rql::{
        self,
        parser::{InsertEvaluationResult, ParserResult},
    },
};

use super::{
    column::Column,
    row::{self, InsertOrUpdateRow},
    storage_providers::traits::StorageIO,
    support_types::ReturnResult,
    table::Table,
};
...
impl<S: StorageIO + Send + Sync> Database<S> {
    pub async fn new(io_sync: S) -> io::Result<Database<S>> {
        ...
        let config_table =
            Table::init("CONFIG_TABLE".into(), config_io_sync.clone(), false, false).await?;
            ...
            let rows = match rows {
                ...
                ReturnResult::HtmlView(_) => {
                    return Err(io::Error::new(
                        ...
                        "Invalid data type returned from query.",
                    ));
                }
            };
    ...
    pub async fn create_table(
        ...
        let new_io_sync = config_table
            .io_sync
            .create_new(format!("{}.db", name))
            .await;
    ...
    pub async fn execute_cached_query(
        &self,
        name: String,
        parser_cached_result: ParserResult,
    ) -> io::Result<Vec<u8>> {
    ...
    pub async fn execute_insert_query(
        &self,
        name: String,
        insert_evaluation_result: InsertEvaluationResult,
    ...
    pub async fn execute_delete_query(
        &self,
        name: String,
        delete_evaluation_tokens: ParserResult,
    ) -> io::Result<()> {
        let mut table = self.tables.try_get_mut(&name);
    ...
    match database_operation.parser_result {
        ...
        ParserResult::InsertEvaluationTokens(insert_evaluation_result) => {
            let rows_affected = database
                .execute_insert_query(database_operation.table_name, insert_evaluation_result)
                .await
                .unwrap();
