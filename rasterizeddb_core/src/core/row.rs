            ...
            Some(return_result) => {
                match return_result {
                    ...
                    ReturnResult::HtmlView(html) => {
                        ...
                        buffer
                            .write_u32::<LittleEndian>(html_bytes.len() as u32)
                            .unwrap();
            ...
            1 => {
                ...
                if buffer.len() < 2 {
                    return Err(Error::new(
                        ...
                        "Buffer too short for type marker",
                    ));
                }
                ...
                match type_marker {
                    0 => {
                        ...
                        for _ in 0..row_count {
                            ...
                            if row_data_end > buffer.len() {
                                return Err(Error::new(
                                    ErrorKind::UnexpectedEof,
                                    "Buffer too short for row data",
                                ));
                            }

                            // Extract row data
                            let columns_data =
                                buffer[position + row_header_size..row_data_end].to_vec();
                    ...
                    1 => {
                        ...
                        if string_end > buffer.len() {
                            return Err(Error::new(
                                ErrorKind::UnexpectedEof,
                                "Buffer too short for HTML data",
                            ));
                        }

                        // Extract and convert string data
                        let html_bytes = &buffer[string_start..string_end];
                        let html_string = String::from_utf8(html_bytes.to_vec()).map_err(|_| {
                            Error::new(ErrorKind::InvalidData, "Invalid UTF-8 in HTML data")
                        })?;
