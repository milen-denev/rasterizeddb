...
pub fn render_rows_to_html(
    ...
    match rows_result {
        Ok(Some(rows)) if !rows.is_empty() => {
            ...
            for row in &rows {
                ...
                let values: Vec<String> = row_columns.iter()
                    .map(|col| col.into_value())
                    .collect();
    ...
    // Render the template
    TEMPLATES.render("table.html", &context)
        .map_err(|e| io::Error::new(io::ErrorKind::Other, format!("Template rendering error: {}", e)))
}
