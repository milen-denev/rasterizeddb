...
#[test]
fn test_custom_protocol_handler() {
    run_async(async {
...
#[test]
fn test_client_disconnect_handling() {
    run_async(async {
    let _ = env_logger::builder().is_test(true).try_init();
    let server_addr = "127.0.0.1";
...
fn test_custom_protocol_implementation() {
    ...
    // For this test, we'll use a simple TCP server without TLS to test the protocol logic
        let listener = compio::net::TcpListener::bind("127.0.0.1:8099").await.unwrap();
    ...
    let server_handle = spawn(async move {
        ...
        // Now test protocol error - write invalid data
            compio::io::AsyncWriteExt::write_all(
                &mut socket,
                b"invalid protocol data without length prefix",
            )
            .await
            .unwrap();
        });
    ...
    // Connect as a client
        let mut stream = compio::net::TcpStream::connect("127.0.0.1:8099").await.unwrap();
